...
- REST 路由支持从 Redfish 连接日志批量导入，自动按日志线程关联 URL、HTTP 方法和多行 JSON 响应；相同方法与 URI 的已有路由会跳过且不会被覆盖。
...
点击路由列表下方的 **Import Logs** 可以粘贴 Redfish 连接日志并批量生成路由。程序从 `##url` 和 `##method` 提取请求，通过日志线程标识关联随后的 `##result` 多行 JSON 响应。解析预览会分别列出可导入、已存在和缺少响应的记录，并按每页 10、20 或 50 条分页展示；翻页不会丢失已选状态。只有可导入记录能够勾选确认，并可使用 **Select All** 或 **Clear Selection** 跨分页批量调整选择。导入响应默认使用状态码 `200` 和响应头 `Content-Type: application/json`。目标分组下拉框默认选择打开导入窗口时当前选中的路由分组，也可以在确认导入前切换为其他已有分组或 `Ungrouped`。相同 HTTP 方法和 URI 的路由始终跳过，不覆盖现有配置。
